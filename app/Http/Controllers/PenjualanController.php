    ...
    function index(Request $request) {
        ...
        if (count($get) > 0) {
            ...
            if ($get['pelanggan_nama'] && $date_start && $date_end) {
                $notas = Nota::whereBetween('created_at', [$date_start, $date_end])
                    ->where('pelanggan_nama', 'like', "%$get[pelanggan_nama]%")
                    ->where('copy', 1)->orderBy('pelanggan_nama')->get();
                $pelanggan = Pelanggan::where('nama', $get['pelanggan_nama'])->first();
                $pelanggan_id = $pelanggan->id;
            } elseif ($get['pelanggan_nama'] && !$date_start && !$date_end) {
                $notas = Nota::where('pelanggan_nama', 'like', "%$get[pelanggan_nama]%")
                    ->where('copy', 1)
                    ->orderBy('id')->get();
