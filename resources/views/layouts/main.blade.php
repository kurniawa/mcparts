          ...
          <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex h-16 items-center justify-between">
              ...
              <div class="-mr-2 flex md:hidden">
                <!-- Mobile menu button -->
                <button type="button" class="inline-flex items-center justify-center rounded-md bg-gray-800 p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" aria-controls="mobile-menu" aria-expanded="false" onclick="$('#mobile-menu').toggle(300)">
          ...
          <div class="md:hidden text-sm">
            <div class="hidden" id="mobile-menu">
              <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
                @foreach ($menus as $menu)
                  <a href="#" class="text-gray-300 hover:bg-gray-700 hover:text-white block rounded-md px-3 py-2 font-medium">{{ $menu['name'] }}</a>
                @endforeach
              </div>
              <div class="border-t border-gray-700 pb-3 pt-4">
                <div class="flex items-center px-5">
                  @if (Auth::user()->profile_picture)
                    <img class="h-8 w-8 rounded-full" src="{{ asset("storage/" . Auth::user()->profile_picture) }}" alt="Profile Picture">
                    <div class="ml-3">
                      <div class="font-medium leading-none text-white">{{ Auth::user()->nama }}</div>
                  ...
                  <div class="ml-3">
                    <div class="font-medium leading-none text-white">{{ Auth::user()->name }}</div>
                    <div class="font-medium leading-none text-gray-400">{{ Auth::user()->email }}</div>
                  </div>
                  @endif
                ...
                <div class="mt-3 space-y-1 px-2">
                  ...
                  @if ($profile_menu['route'] === 'logout')
                    <form action="{{ route($profile_menu['route']) }}" method="post" class="block">
                        @csrf
                        <button type="submit" class="text-left pl-4 py-2 font-medium text-gray-400 hover:bg-gray-700 hover:text-white w-full flex">
                            <span>{{ $profile_menu['name'] }}</span>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 ml-2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15m3 0l3-3m0 0l-3-3m3 3H9" />
                            </svg>
                    ...
                    @else
                    <a href="{{ route($profile_menu['route']) }}" class="block rounded-md px-3 py-2 font-medium text-gray-400 hover:bg-gray-700 hover:text-white" role="menuitem" tabindex="-1">{{ $profile_menu['name'] }}</a>
                  @endif
                  @endforeach
                </div>
              </div>
