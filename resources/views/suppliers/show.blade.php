...
<header class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h1 class="text-xl font-bold tracking-tight text-gray-900">Data Supplier: {{ $supplier->nama }}</h1>
    </div>
  </header>
<main class="mb-9">
    ...
    <div class="flex justify-center text-xs">
        <div class="lg:w-1/2 md:w-3/4 border rounded p-1 bg-white shadow drop-shadow-sm">
            ...
            <div class="flex mt-1">
                ...
                @endif
                    ...
                    <div class="flex justify-end items-center">
                        @if ($supplier_alamats[$key_alamat]->tipe !== 'UTAMA')
                        <form action="{{ route('suppliers.alamat_utama', [$supplier->id, $alamat->id]) }}" method="POST" class="m-0 flex items-center mr-1" onsubmit="return confirm('Jadikan sebagai alamat UTAMA?')">
                            @csrf
                        ...
                        @endif
                        <a href="{{ route('suppliers.alamat_edit', [$supplier['id'], $alamat['id']]) }}" class="bg-slate-400 rounded text-white p-1">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
                                <path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712zM19.513 8.199l-3.712-3.712-8.4 8.4a5.25 5.25 0 00-1.32 2.214l-.8 2.685a.75.75 0 00.933.933l2.685-.8a5.25 5.25 0 002.214-1.32l8.4-8.4z" />
                                <path d="M5.25 5.25a3 3 0 00-3 3v10.5a3 3 0 003 3h10.5a3 3 0 003-3V13.5a.75.75 0 00-1.5 0v5.25a1.5 1.5 0 01-1.5 1.5H5.25a1.5 1.5 0 01-1.5-1.5V8.25a1.5 1.5 0 011.5-1.5h5.25a.75.75 0 000-1.5H5.25z" />
                            </svg>
                        </a>
                        <form action="{{ route('suppliers.alamat_delete', [$supplier->id, $alamat->id]) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus alamat ini?')" class="m-0 ml-1 flex items-center">
                            @csrf
            ...
            {{-- FORM - ADD_ALAMAT --}}
            <form id="form_alamat_add" class="border rounded p-1 mt-1 hidden" action="{{ route('suppliers.alamat_add', $supplier->id) }}" method="POST">
                @csrf
                <table class="w-full">
            ...
            <div class="hidden" id="form_tambah_kontak">
                <div class="inline-block border rounded p-2 mt-1">
                    <form action="{{ route('suppliers.kontak_add', $supplier->id) }}" class="flex items-center" method="POST">
                        @csrf
                        <span>Tipe:</span>
            ...
            {{-- KONTAK TERSEDIA --}}
            <div class="flex mt-1">
                @foreach ($supplier_kontaks as $key_kontak => $kontak)
                @if ($kontak->is_actual)
                ...
                @endif
                    ...
                    <div class="flex justify-end items-center">
                        @if (!$kontak->is_actual)
                        <form action="{{ route('suppliers.kontak_utama', [$supplier->id, $supplier_kontaks[$key_kontak]->id]) }}" method="POST" class="m-0 flex items-center mr-1" onsubmit="return confirm('Jadikan sebagai kontak UTAMA?')">
                            @csrf
                            <button type="submit" class="text-slate-300">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                                    <path fill-rule="evenodd" d="M8.603 3.799A4.49 4.49 0 0112 2.25c1.357 0 2.573.6 3.397 1.549a4.49 4.49 0 013.498 1.307 4.491 4.491 0 011.307 3.497A4.49 4.49 0 0121.75 12a4.49 4.49 0 01-1.549 3.397 4.491 4.491 0 01-1.307 3.497 4.491 4.491 0 01-3.497 1.307A4.49 4.49 0 0112 21.75a4.49 4.49 0 01-3.397-1.549 4.49 4.49 0 01-3.498-1.306 4.491 4.491 0 01-1.307-3.498A4.49 4.49 0 012.25 12c0-1.357.6-2.573 1.549-3.397a4.49 4.49 0 011.307-3.497 4.49 4.49 0 013.497-1.307zm7.007 6.387a.75.75 0 10-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 00-1.06 1.06l2.25 2.25a.75.75 0 001.14-.094l3.75-5.25z" clip-rule="evenodd" />
                        ...
                        <form action="{{ route('suppliers.kontak_delete', $supplier_kontaks[$key_kontak]->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kontak ini?')" class="m-0">
                            @csrf
                    ...
                    {{-- EDIT_KONTAK --}}
                    <form action="{{ route('suppliers.kontak_edit', $supplier_kontaks[$key_kontak]->id) }}" method="POST" class="mt-1 border rounded p-1 hidden" id="form_kontak_edit-{{ $key_kontak }}">
                        @csrf
                        <table>
                            <tr>
                                <td>Tipe</td><td>:</td>
