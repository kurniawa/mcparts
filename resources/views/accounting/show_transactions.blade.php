        ...
        <div class="flex">
            ...
            <div>
                <div class="border rounded p-1 ml-2">
                    <table class="text-xs">
                        <tr><td>Tipe Instansi</td><td>:</td><td><div class="ml-2">{{ $userInstance->instance_type }}</div></td></tr>
                        <tr><td>Nama Instansi</td><td>:</td><td><div class="ml-2">{{ $userInstance->instance_name }}</div></td></tr>
                        <tr><td>Branch</td><td>:</td><td><div class="ml-2">{{ $userInstance->branch }}</div></td></tr>
                        @if ($userInstance->account_number)
                        <tr><td>Nomor Rek.</td><td>:</td><td><div class="ml-2">{{ $userInstance->account_number }}</div></td></tr>
                        @endif
                    </table>
                </div>
            </div>
        ...
        <div class="mt-2">
            <table class="text-xs table-border w-3/4 max-w-full">
                ...
                <tr>
                    ...
                    <td>
                        <div>
                            <form action="{{ route('accounting.up_down_transaction', [$userInstance->id, $accounting->id]) }}" class="flex" method="POST" onsubmit="return confirm('up sekali?')">
                                @csrf
                                <div class="flex">
                        ...
                        <div>
                            <form action="{{ route('accounting.up_down_transaction', [$userInstance->id, $accounting->id]) }}" class="flex" method="POST" onsubmit="return confirm('down sekali?')">
                                @csrf
                                <div>
                ...
                <tr class="hidden" id="tr_edit_transaction-{{ $key_accounting }}">
                    <td colspan="6">
                        <div class="text-center">
                            <form action="{{ route('accounting.edit_entry', [$userInstance->id, $accounting->id]) }}" method="POST" class="inline-block p-1 rounded bg-white shadow drop-shadow">
                                @csrf
                                ...
                                <div class="flex">
                                    ...
                                    <div class="ml-1">
                                        ...
                                        <input type="text" name="kode" id="edit-{{ $key_accounting }}-kode" class="border p-1 text-xs mt-1 w-20" value="{{ $userInstance->kode }}" value="{{ $accounting->kode }}">
                                    </div>
                                    <div class="ml-1">
    ...
    function autocomplete_deskripsi(index) {
        $(`#transaction_desc-${index}`).autocomplete({
            ...
            select: function (event, ui) {
                ...
                accountingGetRelatedInvoice(ui.item.id, index, ui.item.kategori_level_one);
            }
        });
    }
